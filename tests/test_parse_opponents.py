...
def test_parse_opponents_returns_list_of_dicts():
    ...
    result = statistics.parse_opponents(1,data)
    
        # Assert list with 82 games with first, last, and middle game ids
    assert isinstance(result, list)
    assert len(result) == 82
    assert 'Toronto Raptors' == result[0]
    assert 'Detroit Pistons' == result[14]
    assert 'Miami Heat' == result[81]